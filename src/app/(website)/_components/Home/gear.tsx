    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/home/Gear`
        );
        if (!response.ok) {
      ...
      {firstPost && (
        <div className="py-8">
          ...
          >
            ...
            >
              {/* <div className="text-center bg-black bg-opacity-50 !text-white p-6 rounded">
              <Link
                href={`/${firstPost.category_id}/${firstPost.subcategory_id}/${firstPost.id}`}
              >
                <p
                  dangerouslySetInnerHTML={{ __html: firstPost.heading }}
                  className="text-2xl font-medium  text-white dark:text-white hover:underline"
                />
              </Link>
              <p
                dangerouslySetInnerHTML={{ __html: firstPost.body1 }}
                className="text-sm font-normal  text-white dark:text-white line-clamp-3 mt-2"
              />
            </div> */}
            </div>
          </Link>
